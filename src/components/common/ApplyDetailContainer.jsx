...
import styles from "./ApplyDetailContainer.module.scss";

function ApplyDetailContainer() {
  ...
  const boardType = "post_Apply"; // ✅ `post_Apply` 컬렉션으로 변경
  ...
  useEffect(() => {
    const fetchPost = async () => {
      if (!postId) return;
      try {
        const docRef = doc(db, "post_Apply", postId); // ✅ `post_Apply` 컬렉션에서 데이터 가져오기
        const docSnap = await getDoc(docRef);
  
        if (docSnap.exists()) {
          setPost(docSnap.data()); // ✅ 성공하면 데이터 저장
        } else {
